...
var callbacks = {
    ...
    'slaveDispose': function() {
        // We're exiting normally, no error
        console.log("Exiting on slave dispose");
        phantom.exit(0);
    }
};
...
page.onCallback = function(event) {
    ...
    if (handler) {
        ...
        console.log("The page is trying to call an undefined callback: ", event.name);
    }
};
...
function makePageOpenTimeout(expectedEvent) {
    return setTimeout(function() {
        console.log("Timed out after waiting", pageOpenTimeoutMs, "milliseconds for the", expectedEvent, "event");
...
page.open(url, function(status) {
    clearTimeout(pageOpenTimeout);
    if (status !== "success") {
        console.log("Page open error with status:", status);
